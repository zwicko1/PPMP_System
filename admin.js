document.addEventListener("DOMContentLoaded", () => {
      ...
      if (user.role_id === 1) {
        ...
        if (adminNameDisplay)
          adminNameDisplay.innerText =
            "Logged in as: " + (user.unit_name || "Admin");
  ...
  if (adminPasswordInput)
    adminPasswordInput.addEventListener("keydown", triggerAdminLogin);
  if (adminEmailInput)
    adminEmailInput.addEventListener("keydown", triggerAdminLogin);
  ...
  const tabApp = document.getElementById("tabApp"); // NEW
  const tabData = document.getElementById("tabData");

  const budgetPanel = document.getElementById("budgetPanel");
  const ppmpPanel = document.getElementById("ppmpPanel");
  const appPanel = document.getElementById("appPanel"); // NEW
  const dataPanel = document.getElementById("dataPanel");

  function resetTabs() {
    [tabBudget, tabPpmp, tabApp, tabData].forEach((tab) => {
      if (tab) {
        tab.style.background = "transparent";
        tab.style.border = "1px solid #7f8c8d";
      }
    });
    [budgetPanel, ppmpPanel, appPanel, dataPanel].forEach((panel) => {
  ...
  if (tabApp) {
    tabApp.addEventListener("click", () => {
      ...
      tabApp.style.background = "#c14f3b";
      tabApp.style.border = "none";
      
      // Auto-set the year filter to next year
      const appYear = document.getElementById("appYearFilter");
      if (appYear && !appYear.value) appYear.value = new Date().getFullYear() + 1;
    });
  }
  ...
  if (tabBudget && tabPpmp && tabData) {
    ...
    tabData.addEventListener("click", () => {
      ...
      if (typeof window.loadAdminDataTables === "function")
        window.loadAdminDataTables();
    });
  } else {
    console.error(
      "Tab Navigation Error: One of the tab or panel IDs is missing in the HTML!",
    );
  }
  ...
  function recalculateUnallocatedRealTime() {
    const masterStr = document
      .getElementById("masterBudgetInput")
      .value.replace(/[^0-9.-]/g, "");
    ...
    if (unallocatedEl) {
      unallocatedEl.innerText =
        "₱ " +
        Math.abs(unallocated).toLocaleString(undefined, {
  ...
  window.saveMasterBudget = function () {
    ...
    if (numericValue < window.currentAllocatedTotal) {
      ...
      if (masterEl)
        masterEl.value = window.currentAllocatedTotal.toLocaleString(
          undefined,
  ...
  function loadDashboard() {
    ...
    tbody.innerHTML =
      '<tr><td colspan="3" style="text-align: center;">Refreshing...</td></tr>';

    Promise.all([
      fetch("http://127.0.0.1:8000/api/admin/sectors", {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
        },
      }),
      fetch(`http://127.0.0.1:8000/api/admin/budgets?year=${year}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
        },
      }),
      fetch(`http://127.0.0.1:8000/api/admin/master-budget?year=${year}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
        },
      ...
      .then(([sectorsResp, budgetsResp, masterResp]) => {
        ...
        const dbMasterBudget = masterResp.total_amount
          ? parseFloat(masterResp.total_amount)
          : 0;
        const masterBudgetInput = document.getElementById("masterBudgetInput");
        if (masterBudgetInput) {
          masterBudgetInput.value = dbMasterBudget
            ? dbMasterBudget.toLocaleString(undefined, {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })
            : "";
        ...
        const sectorsData = Array.isArray(sectorsResp)
          ? sectorsResp
          : sectorsResp.data || [];
        const budgetsData = Array.isArray(budgetsResp)
          ? budgetsResp
          : budgetsResp.data || [];
        ...
        if (sectorsData.length > 0) {
          ...
          document.getElementById("totalBudgetValue").innerText =
            "₱ " +
  ...
  window.openModal = function (sectorId, sectorName, currentAmount) {
    ...
    document.getElementById("modalYearLabel").innerText =
      document.getElementById("yearFilter").value;

    // Format the existing amount with commas when the modal opens
    let formattedAmount = currentSectorOldAmount
      ? currentSectorOldAmount.toLocaleString(undefined, {
  ...
  if (saveBtn) {
    saveBtn.addEventListener("click", () => {
      ...
      // ----------------------------------------
      const masterBudgetStr = document
        .getElementById("masterBudgetInput")
        .value.replace(/[^0-9.-]/g, "");
      ...
      if (masterBudget <= 0) {
        errorMsg.innerText =
          "Error: Please set and save a Master Institution Budget first.";
        errorMsg.style.display = "block";
        return;
      }

      const availableBalance =
        masterBudget - (window.currentAllocatedTotal - currentSectorOldAmount);

      if (newAmount > currentSectorOldAmount && newAmount > availableBalance) {
        errorMsg.innerText =
          "Allocation Denied: The maximum you can assign to this specific sector is ₱" +
          availableBalance.toLocaleString(undefined, {
            minimumFractionDigits: 2,
          }) +
          " (Unallocated Balance + their current budget).";
        errorMsg.style.display = "block";
        return;
      }
      // ----------------------------------------
  ...
  window.loadAdminPpmps = function () {
    ...
    const tbody =
      document.getElementById("adminPendingTableBody") ||
      document.querySelector("#adminPpmpTable tbody");
    if (!tbody) return;
    ...
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align: center; padding: 40px; color: #6c757d;">Loading dashboard...</td></tr>';
    ...
    fetch("http://127.0.0.1:8000/api/admin/ppmps/all", {
      ...
      .then((data) => {
        ...
        const filteredPpmps = data.data.filter(
          (ppmp) => parseInt(ppmp.fiscal_year) === selectedYear,
        );
        ...
        Object.values(groupedPpmps).forEach((group) => {
          ...
          const sectorName = latest.user
            ? latest.user.unit_name
            : "Unknown Sector";
          const latestDateObj = new Date(latest.created_at);
          const latestNiceDate =
            latestDateObj.toLocaleDateString() +
            " " +
            latestDateObj.toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            });
          ...
          mainRow.style.borderBottom =
            olderRevisions.length > 0 ? "none" : "1px solid #eee";
          ...
          olderRevisions.forEach((old, index) => {
            ...
            oldRow.style.borderBottom =
              index === olderRevisions.length - 1 ? "1px solid #eee" : "none";

            const oldDateObj = new Date(old.created_at);
            const oldNiceDate =
              oldDateObj.toLocaleDateString() +
              " " +
              oldDateObj.toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              });
  ...
  function openReviewModal(ppmpId, mode) {
    ...
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align: center; padding: 40px; color: #6c757d;">Fetching project details...</td></tr>';

    if (mode === "readonly") {
      titleEl.innerText = `Viewing Old Revision (ID: ${ppmpId})`;
      titleEl.style.color = "#6c757d";
      subtitleEl.innerText =
        "This is a superseded version. It is locked for auditing purposes.";
      footerEl.innerHTML = "";
    } else if (mode === "review") {
      titleEl.innerText = `Reviewing Latest PPMP (ID: ${ppmpId})`;
      titleEl.style.color = "#0d6efd";
      subtitleEl.innerText =
        "Carefully review the requested items below before making a decision.";
        ...
        if (data.status === "success" && data.data.items) {
          ...
          data.data.items.forEach((item) => {
            ...
            row.innerHTML = `
                        ...
                        <td style="padding: 12px; border: 1px solid #dee2e6;">${item.category}</td>
                        <td style="padding: 12px; border: 1px solid #dee2e6; color: #495057;">${item.type || 'N/A'}</td>
          ...
          tbody.innerHTML =
            '<tr><td colspan="6" style="text-align: center; color: red; padding: 30px;">Failed to load items.</td></tr>';
        }
      })
      .catch((error) => {
        console.error(error);
        tbody.innerHTML =
          '<tr><td colspan="6" style="text-align: center; color: red; padding: 30px;">Connection error.</td></tr>';
      });
  }
  ...
  window.submitDynamicDecision = function (id) {
    ...
    if (
      statusName.toLowerCase().includes("return") ||
    ...
    if (
      !confirm(
        `Are you sure you want to change this PPMP's status to: ${statusName}?`,
      )
    ) {
      return;
    }
  ...
  window.loadAdminDataTables = function () {
    ...
    // Fetch Types
    fetch("http://127.0.0.1:8000/api/admin/ppmp-types", {
  ...
  function renderDataList(elementId, items, tableType) {
    ...
    items.forEach((item) => {
      ...
      li.style.cssText =
        "display: flex; justify-content: space-between; background: white; padding: 10px; border: 1px solid #ddd; margin-bottom: 8px; border-radius: 4px; align-items: center;";
  ...
  window.updateDataEntry = function (tableType, id) {
    ...
    const nameVal = document
      .getElementById(`edit_${tableType}_${id}`)
      .value.trim();
  ...
  if (toggleAdminPassword && adminPasswordField) {
    toggleAdminPassword.addEventListener("click", function () {
      const type =
        adminPasswordField.getAttribute("type") === "password"
          ? "text"
          : "password";
  ...
  window.toggleGlobalSubmission = function () {
    ...
    fetch("http://127.0.0.1:8000/api/admin/settings/submission-status/toggle", {
      ...
      body: JSON.stringify({ is_open: newState, year: targetYear }),
    })
      .then(async (res) => {
        const data = await res.json();
  ...
  // Auto-fetch settings when Data Management tab is clicked
  const tabDataNav = document.getElementById("tabData");
  if (tabDataNav) {
    tabDataNav.addEventListener("click", () => {
      fetchGlobalSettings();
    });
  }
  ...
  window.generateApp = function () {
    ...
    tbody.innerHTML = '<tr><td colspan="13" style="text-align: center; padding: 40px; color: #0d6efd;">Verifying PPMP Approvals and Compiling Data... ⏳</td></tr>';

    fetch(`http://127.0.0.1:8000/api/admin/app/generate?year=${year}`, {
      headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
    })
      .then((response) => response.json())
      .then((data) => {
        if (data.status === "error") {
            tbody.innerHTML = `<tr><td colspan="13" style="text-align: center; padding: 40px; color: #dc3545; font-weight: bold;">🛑 ${data.message}</td></tr>`;
            return;
        }
      ...
      .catch((err) => {
          ...
          tbody.innerHTML = `<tr><td colspan="13" style="text-align: center; padding: 40px; color: #dc3545; font-weight: bold;">Connection Error. Check console.</td></tr>`;
      });
  };
  ...
  window.exportAppToExcel = function () {
    ...
    // Loop through every row in the generated table
    Array.from(tbody.rows).forEach((row) => {
        // Find all inputs/selects in this row
        const inputs = row.querySelectorAll("input, select");
        
        // If there are no inputs, skip (it might be an error or loading row)
        if (inputs.length === 0) return;

        // Build a clean JSON object reading the exact values the Admin typed
        appData.push({
            "Project Title": inputs[0] ? inputs[0].value : "",
            "End-User/Unit": inputs[1] ? inputs[1].value : "",
            "General Description": inputs[2] ? inputs[2].value : "",
            "Mode of Procurement": inputs[3] ? inputs[3].value : "",
            "Early Procurement?": inputs[4] ? inputs[4].value : "",
            "Bid Evaluation Criteria": inputs[5] ? inputs[5].value : "",
            "Start Date": inputs[6] ? inputs[6].value : "",
            "End Date": inputs[7] ? inputs[7].value : "",
            "Source of Fund": inputs[8] ? inputs[8].value : "",
            "Estimated Budget": row.cells[10] ? row.cells[10].innerText.trim() : "", // The pre-filled budget
            "Strategy/Tools": inputs[9] ? inputs[9].value : "",
    ...
    XLSX.utils.book_append_sheet(wb, ws, `APP FY ${year}`);
    XLSX.writeFile(wb, `TUP_Annual_Procurement_Plan_FY${year}.xlsx`);
  };
  ...
  window.printApp = function () {
      ...
      printTable.style.textAlign = "center";
      
      // Copy the Header
      printTable.innerHTML = originalTable.querySelector("thead").outerHTML;
      
      // Build the Body with flat text
      const tbody = document.createElement("tbody");
      Array.from(document.getElementById("appTableBody").rows).forEach(row => {
          ...
          Array.from(row.cells).forEach(cell => {
              ...
              // If it has an input, print its value. Otherwise, print innerText.
      ...
      const headerHTML = `
        <div style="text-align: center; margin-bottom: 20px;">
            ...
            <h3 style="margin:5px 0;">ANNUAL PROCUREMENT PLAN FOR FY ${year}</h3>
        </div>`;

      const newWin = window.open("", "_blank");
      newWin.document.write(`
        <html><head><title>Print APP</title>
        <style>
          body { font-family: sans-serif; margin: 20px; }
          @media print { @page { size: landscape; margin: 10mm; } }
          th { border: 1px solid #000; padding: 4px; background: #eee; }
        </style>
        </head><body>${headerHTML}${printTable.outerHTML}</body></html>`);
